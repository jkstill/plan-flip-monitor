...
# compare realtime data to historic data
	...
	'unstable-plans-baseline-realtime' => q{select
   r.sql_id
   , r.plan_hash_value
   , r.execs
   , r.avg_lio
   , r.plan_count
   , h.avg_etime hist_etime
   , r.avg_etime real_etime
   , h.stddev_etime
from (
with min_snap_id as
(
...
on r.sql_id = h.sql_id and r.plan_hash_value = h.plan_hash_value
where 
	-- +- 1 stddev
	case when r.avg_etime between h.avg_etime - h.stddev_etime and h.avg_etime + h.stddev_etime
		then 'OK'
		else 'NOT_OK'
	end = 'NOT_OK'
order by r.sql_id, r.plan_hash_value
},
# end of unstable-plans-baseline-CONTAINER
);
